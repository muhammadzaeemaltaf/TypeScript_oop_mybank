...
class Bank {
    ...
    transaction(accountUpdate) {
        let account = this.accounts.find((acc) => acc.accountNumber === accountUpdate.accountNumber);
        if (account) {
...
let customer2 = new Customer("Sarah", "Fatima", 24, "Female", 923335375982, 1002);
...
async function service(bank) {
    do {
        ...
        if (service.option == "View Balance") {
            ...
            if (account) {
                let name = myBank.customers.find((person) => person.accountNumber === account?.accountNumber);
                console.log(`Dear, ${chalk.green.italic(name?.firstName + " " + name?.lastName)}! Your Account Balance is ${chalk.bold.blueBright("$" + account.balance)}`);
            }
        }
        if (service.option == "Cash Withdraw") {
            ...
            if (account) {
                ...
                if (ans.ques > account.balance) {
                    ...
                    console.log(`Dear, ${chalk.green.bold.italic(name?.firstName + " " + name?.lastName)}! Withdrawal of ${chalk.blueBright.bold("$" + ans.ques)} Successful. Remaining balance: ${chalk.blueBright.bold("$" + account.balance)}`);
                }
            }
        }
        if (service.option == "Cash Deposit") {
            ...
            if (account) {
                ...
                console.log(`Dear, ${chalk.green.bold.italic(name?.firstName + " " + name?.lastName)}! Amount ${chalk.blueBright.bold("$" + ans.ques)} Successfully Deposit. Remaining balance: ${chalk.blueBright.bold("$" + account.balance)}`);
                if (ans.ques > 100) {
